...
elif opt.dataset =='ecommerce':
    dataset = 'Oct-processed.csv' 
...
with open(dataset, "r") as f:
    ...
    for data in reader:
        ...
        if opt.dataset == 'yoochoose':
            ...
            item = data['product_id']#, int(data['timeframe'])
        else:
...
if opt.dataset == 'yoochoose':
    ...
    splitdate = 1.1864214803320454e+16 
else:
    splitdate = maxdate - 86400 * 7

print('Splitting date', splitdate)      # Yoochoose: ('Split date', 1411930799.0)
tra_sess = filter(lambda x: x[1] < splitdate, dates)
print('min',np.min(dates))
print('mean',np.mean(dates))
print('max',np.max(dates))
...
tes_sess = tra_sess[:52185]
tra_sess = tra_sess [52185:]                    
print(tra_sess[:3])
print(tes_sess[:3])
...
if opt.dataset == 'diginetica':
    ...
    pickle.dump(tra, open('ecommerce/train.txt', 'wb'))
    pickle.dump(tes, open('ecommerce/test.txt', 'wb'))
    pickle.dump(tra_seqs, open('ecommerce/all_train_seq.txt', 'wb'))
elif opt.dataset == 'yoochoose':
